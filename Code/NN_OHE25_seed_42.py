...
3. Instead of the embedding layer we ohe each categrical variable and then concatenate them , then apply a linear + relu on it ( reducing the output to 1461(total unique categorical states )//(0.75)
...
#------Result File ----------
results_file = f"BTS/Pytorch_embeddings_jobs/output_paper1_results/emb_0_arch_4/results_seed_{seed}.txt"  

#---- Summary Writer for logs 
# The base directory for tensorboard logs
base_dir = f"/home/aniketb/scratch/saved/logs_output_paper1/emb_0_arch_4/{seed}"
log_dir = f"{base_dir}/"
...
class TabularModel(nn.Module):
    def __init__(self, cat_dims, n_cont):  # cat_dims: List of unique values per categorical column
        ...
        self.lin_cat = nn.Linear(self.n_cat, self.n_cat * 3 // 4)  # Output size is 3/4 th of the input size
        
        # Fully connected layers
        self.lin1 = nn.Linear(self.n_cat * 3 // 4 + self.n_cont, 200)
